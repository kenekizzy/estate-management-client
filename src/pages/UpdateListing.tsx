...
const UpdateListing = () => {
  ...
  const fetchListingData = async () => {
    await axios.get(`${url}/v1/listing/get-listing/${id}`).then((response) => {
        console.log("Listings", response.data.listing)
      setListing(response.data.listing)
      setName(response.data.listing.name)
      setDescription(response.data.listing.description)
      setAddress(response.data.listing.address)
      setType(response.data.listing.type)
      setBedrooms(response.data.listing.bedrooms)
      setBathrooms(response.data.listing.bathrooms)
      setRegularPrice(response.data.listing.regularPrice)
      setDiscountPrice(response.data.listing.discountPrice)
      setOffer(response.data.listing.offer)
      setParking(response.data.listing.parking)
      setFurnished(response.data.listing.furnished)
      setImageUrls(response.data.listing.imageUrls)
    }).catch((err) => {
      console.log(err)
    })
  }
  ...
  const handleSubmit = async(e) => {
    ...
    await axios.put(`${url}/v1/listing/update-listing/${listing._id}`, formData).then(response => {
    ...
    <main className="p-3 max-w-4xl mx-auto">
        ...
        <form className="flex flex-col gap-4 sm:flex-row" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col gap-4 flex-1">
                ...
                <button type="submit" className="p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-80">Update Listing</button>
            </div>
        </form>
    </main>
  )
